...
=ВПР(F6;$A$7:$B$14;2)
  ...
  <details>
  <summary>[180] Т-банк второй этап (нужно будет потом решить - метрики, АБ, кейс)</summary> вс
...
<details>
<summary>[179] Bell Integrator</summary> пн
...
SELECT count(promocode_id)/count(*) from orders

-- 2) Какой самый популярный промокод и число его использований?

SELECT name, count(order_id) as cnt
from orders join promocodes using (promocode_id)
group by name
order by cnt desc
limit 1
...
1) Какими способами можно избавиться от дубликатов?

- через row_number (и потом отфильтровать строки, где row_number > 1)
- через группировку всех столбцов
- через джойн саму на себя и условие
- через distinct
- через exists

 ```sql
-- Удалить дубликаты и вывести только те строки, где были дубли

with tmp as(SELECT title, author, format,
count() over (partition by title, author, format) as cnt
from books)
select * from tmp
where cnt > 1
```
...
apply позволяет применить функцию к столбцу/строке датафрейма и series
...
<details>
<summary>[178] Rocket Bank</summary> вт

# [1] Задача 

 ```sql

with tmp as (select user_id, count(event_id) as cnt
from app_events
where event_time >= current_date - interval('1 month')
...
with tmp as (select user_id, event_id, event_type, event_time,
lag(event_time) over (partition by user_id order by event_time) as prev_login
from sandbox.zvereva_app_events
where event_type = 'login' and event_time >= current_date - interval '3 month' and event_time<current_date),

tmp_2 as (select user_id, 
avg(EXTRACT(EPOCH FROM (event_time - prev_login))/60) as diff
from tmp
WHERE prev_login IS NOT NULL
group by user_id)

select percentile_cont(0.5) within group (order by diff) from tmp_2
```
...
<details>
<summary>[177] Your Personal Agent</summary> ср

# [1] Задача 

 ```sql

-- Написать скрипт SQL, позволяющий вычислить среднюю дельту по пользователям между первым и вторым депозитом для пользователей, кто совершил минимум 2 депозита

with tmp as (
  select userId, createdAt,
  row_number() over (partition by userId order by createdAt) as rn,
  count(createdAt) over (partition by userId) as cnt
  from tranzes_logs
  where isError = False and type = 'deposit'),

  tmp_2 as (select t1. userId,
  extract (epoch from (t2.createdAt - t1.createdAt)) as diff
  from tmp t1 join tmp t2 on t1.userId = t2.userId
  and t1.rn = 1
  and t2.rn = 2
  where cnt >= 2
)
...
# [2] Задача 
 ...
 ```sql
-- Написать скрипт SQL, вычисляющий  долю средств, которые пользователи проиграли от совершенных депозитов за последние 12 часов

with tmp_1 as (
  select sum(amount)
  from tranzes_logs
  where isError = False and type = 'deposit' 
  and createdAt > current_timestamp - interval '12 hours'
),
tmp_2 as(
  select sum(amount * usdCurrencyRate)
  from win_log
  where createdAt > current_timestamp - interval '12 hours'
)
select 
((select * from tmp_1) - (select * from tmp_2))/(select * from tmp_1)
```
...
Монетка 1: 0.5
Монетка 2: 0.333
поэтому первая монетка более везучая

# [3] Задача 

 ```python
  '''Графики нормальных распределений'''
...
pdf_cat = stats.norm.pdf (x, mean_cat, std_cat)
pdf_dog = stats.norm.pdf (x, mean_dog, std_dog)

plt.plot(x, pdf_cat)
plt.plot(x, pdf_dog)

'''Графики z-нормализации'''

z = (x - mean_cat)/std_cat
...
from employeedetails ed where empid not in (
...
# [5] Задача 
 ...
 ```sql
select project, count (empid) as cnt
from employeesalary
group by project
order by cnt desc
```
...
# [6] Задача 
 ...
 ```sql
with tmp as (select project, count (empid) as cnt
from employeesalary
group by project)
select project, cnt,
